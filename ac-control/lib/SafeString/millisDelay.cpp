...
/**
   ...
   @param delay in milliseconds, 0 means justFinished() will return true on first call
*/
void millisDelay::start(const unsigned long delay) {
  ms_delay = delay;
  startTime = millis();
  running = true;
  finishNow = false; // do not finish early
...
*/
bool millisDelay::isRunning() const {
  return running;
}
...
*/
unsigned long millisDelay::getStartTime() const {
  return startTime;
}
...
*/
unsigned long millisDelay::remaining() const {
  if (running) {
    const unsigned long ms = millis(); // capture current millis() as it may tick over between uses below
...
*/
unsigned long millisDelay::delay() const {
  return ms_delay;
}
