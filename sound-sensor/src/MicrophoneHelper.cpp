...
MicrophoneHelper::MicrophoneHelper(const int i2sClockPin, const int i2sWsPin, const int i2sDataInPin):
    ...
    sampleRate(8500),
    i2sPort(I2S_NUM_0),
    FFT(ArduinoFFT<double>(vReal, vImag, SAMPLES, sampleRate)) {}
...
void MicrophoneHelper::getBands(Bands &bands) {
  ...
  size_t bytesIn = 0;
  const esp_err_t result = i2s_read(i2sPort, &rawData, SAMPLES, &bytesIn, portMAX_DELAY);
  if (result != ESP_OK) {
#if DEBUG
  ...
  FFT.windowing(vReal, SAMPLES, FFTWindow::Rectangle, FFTDirection::Forward);
